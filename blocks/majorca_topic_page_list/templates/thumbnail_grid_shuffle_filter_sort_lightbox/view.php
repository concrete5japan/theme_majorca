...
<div id="shuffle-grid-<?php echo $bID; ?>" class="grid-card">
    ...
    <?php foreach ($pages as $page):
		...
		$noImgSrc = $this->getBlockURL(). '/img/no_image.png';
        ?>

	    <div data-groups='["<?php echo t('All')?>"<?php $topics = $page->getAttribute($attributeHandle);if (isset($topics) && count($topics)) { foreach ($topics as $topic) { echo ',"' .$topic->getTreeNodeDisplayName(). '"'; ?><?php } } ?>]' data-date-created="<?php echo $date?>" data-title="<?php echo $title; ?>" class="grid-item lightbox-pagelist thumbnail-grid-shuffle">
